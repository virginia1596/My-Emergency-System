...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql1 = "INSERT INTO medical_requests (full_name, phone_number, emergency_type, location, additional_details, file_path)
             VALUES (?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt1 && $stmt2) {
        ...
        if ($stmt1->execute() && $stmt2->execute()) {
            echo "<script>alert('✅ Medical emergency request submitted and added to dashboard!'); window.location.href='Medical.html';</script>";
            exit;
        } else {
...
<head>
  <meta charset="UTF-8">
  <title>Medical Emergency Request</title>
  <style>
    body {
