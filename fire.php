...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql1 = "INSERT INTO fire_requests (full_name, phone_number, emergency_type, location, additional_details, file_path)
             VALUES (?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt1 && $stmt2) {
        ...
        if ($stmt1->execute() && $stmt2->execute()) {
            echo "<script>alert('✅ Fire emergency request submitted and added to dashboard!'); window.location.href='Fire.html';</script>";
            exit;
        } else {
...
<head>
  <meta charset="UTF-8">
  <title>Fire Emergency Request</title>
  <style>
    body {
...
<body>
  ...
  <h1>🔥 Request Fire Emergency Assistance</h1>
  <form action="fire.php" method="POST" enctype="multipart/form-data">
    ...
    <label for="emergency_type">Type of Fire Emergency</label>
    <select id="emergency_type" name="emergency_type" required>
      <option>House Fire</option>
      <option>Market Fire</option>
